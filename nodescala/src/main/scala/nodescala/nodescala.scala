...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val listsub: Subscription = listener.start
    val sub: Subscription = Future.run()(ct => async {
      while (ct.nonCancelled)
        await(listener.nextRequest) match { case (req, exc) => respond(exc, ct, handler(req)) }
      listsub.unsubscribe
    })
    ...
    def nextRequest(): Future[(Request, Exchange)] = {
      ...
      blocking {
        createContext(exchange =>
          Future.always((exchange.request, exchange)) onComplete {
            removeContext
            p.complete(_)
          })
      }
