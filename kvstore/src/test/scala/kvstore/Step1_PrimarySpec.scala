...
class Step1_PrimarySpec extends TestKit(ActorSystem("Step1PrimarySpec"))
  ...
  test("case2: Primary (in isolation) should react properly to Insert, Remove, Get") {
    ...
    test("case1: Primary (in isolation) should properly register itself to the provided Arbiter - f") {
    val arbiter = TestProbe()
    system.actorOf(Replica.props(arbiter.ref, Persistence.props(flaky = true)), "case1-primary-f")

    arbiter.expectMsg(Join)
  }

  test("case2: Primary (in isolation) should react properly to Insert, Remove, Get -f ") {
    val arbiter = TestProbe()
    val primary = system.actorOf(Replica.props(arbiter.ref, Persistence.props(flaky = true)), "case2-primary-f")
    val client = session(primary)
