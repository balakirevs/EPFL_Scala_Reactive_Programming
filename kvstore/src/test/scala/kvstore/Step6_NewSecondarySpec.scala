...
class Step6_NewSecondarySpec extends TestKit(ActorSystem("Step6NewSecondarySpec"))
  ...
  test("case1: Primary must start replication to new replicas when persistence is flaky") {
    val arbiter = TestProbe()
    val primary = system.actorOf(Replica.props(arbiter.ref, Persistence.props(flaky = true)), "case1-primary-flaky")
    val user = session(primary)
    val secondary = TestProbe()
  ...
  test("case3: Primary must stop replication to removed replicas and waive their outstanding acknowledgements - flaky") {
    val arbiter = TestProbe()
    val primary = system.actorOf(Replica.props(arbiter.ref, Persistence.props(flaky = true)), "case3-primary-flaky")
    val user = session(primary)
    val secondary = TestProbe()
