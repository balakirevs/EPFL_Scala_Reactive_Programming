...
class Step5_PrimaryPersistenceSpec extends TestKit(ActorSystem("Step5PrimaryPersistenceSpec"))
  ...
  test("case4: Primary generates failure after 1 second if global acknowledgement fails - flaky") {
    val arbiter = TestProbe()
    val persistence = TestProbe()
    val primary = system.actorOf(Replica.props(arbiter.ref, Persistence.props(flaky = true)), "case4-primary-flaky")
    val secondary = TestProbe()
    val client = session(primary)
  ...
  test("case5: Primary acknowledges only after persistence and global acknowledgement - flaky") {
    val arbiter = TestProbe()
    val persistence = TestProbe()
    val primary = system.actorOf(Replica.props(arbiter.ref, Persistence.props(flaky = true)), "case5-primary-flaky")
    val secondaryA, secondaryB = TestProbe()
    val client = session(primary)
